...
'''TODO: define an env variable to be the seed file . E.g. S1_OCN2BUFR_SEED abd 
and read the env variable
'''
alongTrackResolution=config.ini.get('ocnVariable','alongTrackResolution')
...
seed_filename=prjFolder+'/conf/seed.bufr'
unexpandedDescriptors = [1007, 2019, 1096, 25061, 5071,5072,5073,5074,5075, 5040,8075, 301011, 301013, 301021, 1012,  7002, 22063, 8012, 2104, 21105,42008, 25103, 25104, 25105, 25106, 25107,  25108, 11001, 11002, 42006, 21030, 201130, 202129, 22022, 202000, 201000, 2026, 2027, 3025, 3026, 40039,40040,40041,40042, 2111, 25014, 25189,  106000, 31001, 42010, 42001, 42002, 42003,42004,42005, 113000, 31001, 5030, 201130,  6030, 201000, 201131,  21135, 201000 , 21136, 201130, 22161, 201000 ,42009,42007]
...
class  netcdfToBufr(object):
    def __init__(self,nc_filename='#',pathOutput='#',crc='#'):
        ...
        if (self.nc_filename!='#' and pathOutput!='#'and crc!='#'):
            ...
            self.output_filename=pathOutput+'/'+(os.path.basename(self.nc_filename).replace('.nc','_'+crc+'.bufr') ).replace('ocn','mbu')
        ...
        for v in self.sentinel1.variables:
            if v=='rvlZeroDopplerTime' : #and np.ma.is_masked(self.sentinel1.variables['rvlZeroDopplerTime'][:]) :
               #print (self.sentinel1.variables[v])
        ...
        #nc_attrs = sentinel1.ncattrs()
        #print(nc_attrs)
        self.dic_attr_value={}
        for a in self.sentinel1.ncattrs():
            self.dic_attr_value[a]=repr(self.sentinel1.getncattr(a))
            #print '\t%s:' % nc_attr, repr(sentinel1.getncattr(nc_attr))
        ...
        if(self.dic_attr_value['polarisation'][2:4]=='HH'):
            self.polarization=0
        elif(self.dic_attr_value['polarisation'][2:4]=='VV'):
            self.polarization=1
        ...
        self.stateVectorVel=(self.dic_attr_value['statevectorVel']).replace('array([','').replace('])','').split(",")
        if(float(self.stateVectorVel[2])>0):
            self.orbitQualifier=0
        elif(float(self.stateVectorVel[2])<0):
    ...
    def oswNetcdf2bufr(self):
        #print '----oswNetcdf2bufr---'
        self.delayedDescriptorReplication = [len(self.dic_dim_value['oswPartitions']),len(self.dic_dim_value['oswAngularBinSize'])]
        ...
        ecc.codes_set(self.bufr, 'typicalYear', int(self.dic_attr_value['firstMeasurementTime'][2:6]))
        ecc.codes_set(self.bufr, 'typicalMonth', int(self.dic_attr_value['firstMeasurementTime'][7:9]))
        ecc.codes_set(self.bufr, 'typicalDay', int(self.dic_attr_value['firstMeasurementTime'][10:12]))
        ecc.codes_set(self.bufr, 'typicalHour', int(self.dic_attr_value['firstMeasurementTime'][13:15]))
        ecc.codes_set(self.bufr, 'typicalMinute', int(self.dic_attr_value['firstMeasurementTime'][16:18]))
        ecc.codes_set(self.bufr, 'typicalSecond', int(self.dic_attr_value['firstMeasurementTime'][19:21]))
        ...
        ecc.codes_set(self.bufr, 'stationAcquisition', (self.dic_attr_value['processingCenter'][2:5]))
        ecc.codes_set(self.bufr, 'softwareVersionNumber', (self.dic_attr_value['oswAlgorithmVersion'][2:]))
        ...
        ecc.codes_set(self.bufr, 'orbitNumber', int(self.dic_attr_value['sourceProduct'][51:57]))
        ecc.codes_set(self.bufr, 'orbitQualifier', self.orbitQualifier)
        ecc.codes_set(self.bufr, 'year', int(self.dic_attr_value['firstMeasurementTime'][2:6]))
        ecc.codes_set(self.bufr, 'month', int(self.dic_attr_value['firstMeasurementTime'][7:9]))
        ecc.codes_set(self.bufr, 'day', int(self.dic_attr_value['firstMeasurementTime'][10:12]))
        ecc.codes_set(self.bufr, 'hour', int(self.dic_attr_value['firstMeasurementTime'][13:15]))
        ecc.codes_set(self.bufr, 'minute', int(self.dic_attr_value['firstMeasurementTime'][16:18]))
        ecc.codes_set(self.bufr, 'second', int(self.dic_attr_value['firstMeasurementTime'][19:21]))
        ...
        ecc.codes_set_double_array(self.bufr, 'directionOfMotionOfMovingObservingPlatform',self.dic_var_value['oswHeading'][0])
        ...
        #print self.alongTrackResolution
        ...
        #print ('Loop replicating 6 descriptors related to the number of partitions ')   
        for p in xrange(len(self.dic_dim_value['oswPartitions'])):
            ecc.codes_set(self.bufr, "#%d#partitionNumber"%(p+1),int(p+1))
            ecc.codes_set(self.bufr, '#%d#dominantSwellWaveDirectionOfSpectralPartition'%(p+1),float(self.dic_var_value['oswDirmet'][0][0][p]))
            ecc.codes_set(self.bufr, '#%d#significantSwellWaveHeightOfSpectralPartition'%(p+1),float(self.dic_var_value['oswHs'][0][0][p]))
            ecc.codes_set(self.bufr, '#%d#dominantSwellWavelengthOfSpectralPartition'%(p+1),float(self.dic_var_value['oswWl'][0][0][p]))
            ecc.codes_set(self.bufr, '#%d#confidenceOfInversionForEachPartitionOfSwellWaveSpectra'%(p+1),float(self.dic_var_value['oswIconf'][0][0][p]))
            ecc.codes_set(self.bufr, '#%d#ambiguityRemovalFactorForSwellWavePartition'%(p+1),float(self.dic_var_value['oswAmbiFac'][0][0][p]))
        
        #print ("#########################print oswPartition################")
        ...
        #print (self.dic_var_value['oswPartitions'].shape)    
        #print (np.min(dataPartition), np.max(dataPartition))
        for a in xrange(len(self.dic_dim_value['oswAngularBinSize'])):
            ...
            ecc.codes_set_double_array(self.bufr, "#%d#realPartOfCrossSpectraPolarGridNumberOfBins"%(a+1),self.dic_var_value['oswQualityCrossSpectraRe'][0][0][a][:60])
            ecc.codes_set_double_array(self.bufr, "#%d#imaginaryPartOfCrossSpectraPolarGridNumberOfBins"%(a+1),self.dic_var_value['oswQualityCrossSpectraIm'][0][0][a][:60])
            ecc.codes_set_double_array(self.bufr, "#%d#waveSpectra"%(a+1),self.dic_var_value['oswPolSpec'][0][0][a][:60])
            ecc.codes_set_double_array(self.bufr, "#%d#binPartitionReference"%(a+1),self.dic_var_value['oswPartitions'][0][0][a][:60])
            ecc.codes_set(self.bufr, "#%d#shortestOceanWavelengthOnSpectralResolution"%(a+1),float(self.dic_var_value['oswSpecRes'][0][0][a]))
...
def main():
    ...
    parser.add_argument("--nc2bufr",dest="nc2bufr",nargs='+',help="convert a WV OCN .nc in bufr file. Please insert in input: filename_nc_input, folder_bufr_out, crc_nc_input")
    ...
    if args.nc2bufr:
        ...
        if len(args.nc2bufr) < 3:
            print >>sys.stderr, 'Usage: ', sys.argv[0], ' nc_input  bufr_out crc'
            sys.exit(1)
